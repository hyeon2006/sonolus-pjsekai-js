...
export class FlatNote extends Note {
    ...
    updateParallel() {
        ...
        if (this.despawn)
            return;
        if (time.scaled < this.visualTime.min)
            return;
    ...
    playLinearNoteEffect() {
        particle.effects.spawn(this.linearEffectId, linearEffectLayout({
            lane: this.import.lane,
            shear: 0,
        }), 0.5, false);
    }
    playCircularNoteEffect() {
        particle.effects.spawn(this.circularEffectId, circularEffectLayout({
            lane: this.import.lane,
            w: 1.75,
            h: 1.05,
        }), 0.6, false);
